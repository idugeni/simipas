...
export class UserInterface {
  static async getUserInput(): Promise<UserInput> {
    while (true) {
      ...
      const isConfirmed = await this.askToContinue(
        "Apakah Anda yakin tanggal sudah benar? (y/n): ",
      );
      if (isConfirmed) {
        return userInput;
      } else {
  ...
  private static async getDateInput(): Promise<string> {
    ...
    while (true) {
      const input = await askQuestion(
        rl,
        "Silakan masukkan tanggal pengisian jurnal harian (format DD/MM/YYYY): ",
      );
      if (this.isValidDate(input)) {
        ...
        logger.error(
          `Input tanggal tidak valid, yaitu ${input}. Harap masukkan tanggal dengan format DD/MM/YYYY`,
        );
      }
    }
  ...
  private static async getStartDateInput(): Promise<string> {
    ...
    while (true) {
      const input = await askQuestion(
        rl,
        "Silakan masukkan tanggal mulai (format DD/MM/YYYY): ",
      );
      if (this.isValidDate(input)) {
        validDate = input;
        break;
        ...
        logger.error(
          `Input tanggal mulai tidak valid, yaitu ${input}. Harap masukkan tanggal dengan format DD/MM/YYYY`,
        );
      }
    }
  ...
  private static async getEndDateInput(startDate: string): Promise<string> {
    ...
    while (true) {
      const input = await askQuestion(
        rl,
        "Silakan masukkan tanggal berakhir (format DD/MM/YYYY): ",
      );
      if (!this.isValidDate(input)) {
        logger.error(
          `Input tanggal berakhir tidak valid, yaitu ${input}. Harap masukkan tanggal dengan format DD/MM/YYYY`,
        );
        continue;
      }
      ...
      if (end < start) {
        logger.error(
          `Tanggal berakhir ${input} tidak boleh lebih awal dari tanggal mulai ${startDate}`,
        );
        continue;
      }
      validDate = input;
  ...
  private static async getShiftInput(): Promise<
    "Siang" | "PagiMalam" | "LepasMalam" | "ALL"
  > {
    const rl = createReadlineInterface();
    logger.info(
      "Pilih shift yang diinginkan:\n1. Shift Siang\n2. Shift Pagi dan Malam\n3. Lepas Piket Malam\n4. Semua Shift",
    );
    const shiftAnswer = await askQuestion(
      rl,
      "Silakan masukkan nomor pilihan (1-4): ",
    );
    rl.close();
    ...
    logger.error(
      "Input tidak valid. Harap coba kembali dengan memilih nomor yang sesuai",
    );
