...
export class JournalManager {
  ...
  async navigateToJournal(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error) {
        logger.error(`Navigasi ke halaman jurnal gagal karena ${error.message} sehingga proses tidak dapat dilanjutkan`);
      } else {
        logger.error('Navigasi ke halaman jurnal gagal karena alasan yang tidak diketahui sehingga proses tidak dapat dilanjutkan');
      }
      throw error;
    }
  }
  ...
  async selectDate(date: string): Promise<void> {
    ...
    try {
      ...
      await this.page.waitForSelector('.datepicker', { state: 'visible', timeout: DEFAULT_TIMEOUT });
      ...
      const navigateToTargetHeader = async () => {
        ...
        while (attempts < 12) {
          ...
          if (header.includes(englishMonths[desiredMonth - 1]) && header.includes(String(desiredYear))) break;
      ...
      if (error instanceof Error) {
        logger.error(`Pemilihan tanggal gagal karena ${error.message} sehingga proses pemilihan tidak dapat diteruskan`);
      } else {
        logger.error('Pemilihan tanggal gagal karena alasan yang tidak diketahui sehingga proses pemilihan tidak dapat diteruskan');
      }
      throw error;
    }
  }
  ...
  async fillJournalEntry(entry: JournalEntry): Promise<void> {
    logger.info(`Memproses pengisian entri jurnal dari jam mulai ${entry.startTime} hingga jam selesai ${entry.endTime}`);
    try {
      ...
      if (error instanceof Error) {
        logger.error(`Pengisian entri jurnal gagal karena ${error.message} sehingga proses pengisian tidak dapat dilanjutkan`);
      } else {
        logger.error('Pengisian entri jurnal gagal karena alasan yang tidak diketahui sehingga proses pengisian tidak dapat dilanjutkan');
      }
      throw error;
    }
  }
