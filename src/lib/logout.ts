...
export async function doLogout(page: Page): Promise<void> {
  try {
    ...
    const dialogPromise = page
      .waitForEvent("dialog", { timeout: DEFAULT_TIMEOUT })
      .catch(() => null);
    ...
    if (dialog) {
      logger.info(
        `Dialog muncul dengan pesan "${dialog.message()}" dan akan diterima`,
      );
      try {
        await dialog.accept();
      } catch (error: unknown) {
        ...
        logger.warn(
          `Gagal menerima dialog karena: ${errorMessage}. Proses logout tetap dilanjutkan`,
        );
      }
    } else {
      logger.info(
        "Tidak ada dialog logout yang muncul, melanjutkan proses logout.",
      );
    }
    ...
    logger.error(
      `Terjadi kesalahan saat logout: ${errorMessage}. Proses logout tidak dapat diselesaikan`,
    );
  }
}
